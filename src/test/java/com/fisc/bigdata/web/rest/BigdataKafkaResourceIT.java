package com.fisc.bigdata.web.rest;

import com.fisc.bigdata.config.KafkaProperties;
...
class BigdataKafkaResourceIT {
    ...
    void setup() {
        ...
        BigdataKafkaResource kafkaResource = new BigdataKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bigdata-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bigdata-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
